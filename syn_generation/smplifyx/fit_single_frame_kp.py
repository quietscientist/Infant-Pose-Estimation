...
# Modified by Melanie Segado (msegado@seas.upenn.edu)
...
sys.path.append('/workspaces/Infant-Pose-Estimation/syn_generation/human_body_prior')
...
def fit_single_frame(dims,
    ...
    #try with focal length 500, if no luck, try 1000 then 5000 
    fitted = False
    focal_length_guesses = [100]

    for fl in focal_length_guesses:

        if not fitted: 
            print(f"trying focal length {fl}")

            init_t = fitting.guess_init(body_model, gt_joints, edge_indices,
                                        use_vposer=use_vposer, vposer=vposer,
                                        pose_embedding=pose_embedding,
                                        model_type=kwargs.get('model_type', 'smpl'),
                                        focal_length=fl, dtype=dtype)
            ...
            #monitor = fitting.FittingMonitor(batch_size=batch_size, visualize=visualize, **kwargs)
            with fitting.FittingMonitor(
                ...
                if interactive:
                    ...
                    tqdm.write(f'Camera initialization final loss {cam_init_loss_val}')

                #if cam_init_loss_val is None and fl < 5000: 
                #    print("Retrying with different focal length")
                #    continue
                #elif cam_init_loss_val is None and fl == 5000:
                if cam_init_loss_val is None:
                    raise ValueError("Failed to fit camera")
                    ...
                    with open(result_fn, 'wb') as result_file:
                        ...
                        print(f"now dumping pkl file for {min_idx}")
                        pickle.dump(results[min_idx]['result'], result_file, protocol=4)
                        print("done")
                    ...
                    if save_meshes or visualize:
                        ...
                        out_mesh.apply_transform(rot) ## COMMENTED TO AVOID ROTATING THE MESH
