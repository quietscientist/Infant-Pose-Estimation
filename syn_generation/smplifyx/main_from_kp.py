...
# Modified by Xiaofei Huang (xhuang@ece.neu.edu) and Nihang Fu (nihang@ece.neu.edu) and Melanie Segado (msegado@seas.upenn.edu)
# ------------------------------------------------------------------------------
...
from data_parser_from_kp import create_dataset
from fit_single_frame_kp import fit_single_frame
...
def main(**args):
    ...
    keyp_folder = args.pop('keyp_folder', 'keypoints')
    dataset_obj = create_dataset(keyp_folder=keyp_folder, **args)
    ...
    for idx, data in enumerate(dataset_obj):
        ...
        filename = os.path.basename(f'{data["keyp_path"]}')
        filename_without_extension = os.path.splitext(filename)[0]
        parts = filename_without_extension.split('/')
        desired_part = parts[-1]
        

        print(f'Processing: {desired_part}')
        ...
        for person_id in range(keypoints.shape[0]):
            ...
            curr_result_fn = osp.join(result_folder,
                                      f'{desired_part}.pkl')
            curr_mesh_fn = osp.join(mesh_folder,
                                    f'{desired_part}.obj'.format(desired_part))

            #curr_img_folder = osp.join(output_folder, 'images', fn,
            #                           '{:03d}'.format(person_id))
            
            # Store the arguments for the current experiment
            conf_fn = osp.join(result_folder, f'conf.yaml')
            ...
            for frame_idx in range(0, n_frames, 4):
                try:
                    print(f'now fitting frame {frame_idx}')
                    curr_result_fn = osp.join(result_folder,
                                            f'{desired_part}_{frame_idx}.pkl')
                    curr_mesh_fn = osp.join(mesh_folder,
                                            f'{desired_part}_{frame_idx}.obj')
                    if not os.path.exists(curr_mesh_fn):
                        fit_single_frame(dims,
                                        keypoints[[frame_idx]],
                                        #keypoints[[person_id]],
                        ...
                        with open(os.path.join(os.path.dirname(os.path.normpath(output_folder)),'win_log.txt'), 'a') as file:
                            file.write(desired_part + '\n')
                    else: 
                        print(f'frame {frame_idx} already fitted for {desired_part}')      
